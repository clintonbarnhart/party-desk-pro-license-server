...
final class PDP_Product_Manager {
    ...
    public static function maybe_seed() {
        if (!get_option('pdp_phase5_seeded')) {
            self::activate();
            update_option('pdp_phase5_seeded', '1', false);
        }
    }
    ...
    public static function register_types() {
        ...
        register_post_type(self::RELEASE_TYPE, array(
            'labels' => array('name'=>'Releases','singular_name'=>'Release','add_new_item'=>'Publish New Release','edit_item'=>'Edit Release'),
            'public' => false,
            'show_ui' => true,
            'show_in_menu' => false,
    ...
    public static function meta_boxes() {
        ...
        add_meta_box('pdp_release_details', 'Release Package & Settings', array(__CLASS__, 'release_box'), self::RELEASE_TYPE, 'normal', 'high');
    }

    public static function enable_file_uploads($post) {
        if ($post instanceof WP_Post && $post->post_type === self::RELEASE_TYPE) {
    ...
    public static function release_box($post) {
        ...
        $plugin_name = get_post_meta($post->ID, '_pdp_release_plugin_name', true);
        $filename = get_post_meta($post->ID, '_pdp_release_filename', true);
        $sha256 = get_post_meta($post->ID, '_pdp_release_sha256', true);
        $downloads = absint(get_post_meta($post->ID, '_pdp_release_downloads', true));
        $products = get_posts(array('post_type'=>self::PRODUCT_TYPE,'post_status'=>'publish','numberposts'=>-1,'orderby'=>'title','order'=>'ASC'));

        echo '<div class="pdp-release-builder">';
        echo '<p><strong>Publish a plugin release without using a Media Library attachment ID.</strong> Select the ZIP directly from your computer. Party Desk Pro validates the archive, detects its plugin name and version, and stores it for secure licensed downloads.</p>';
        echo '<table class="form-table"><tr><th><label for="pdp_release_product">Product</label></th><td><select id="pdp_release_product" name="pdp_release_product" required><option value="0">Select product</option>';
        foreach ($products as $item) {
            echo '<option value="'.absint($item->ID).'" '.selected($product,$item->ID,false).'>'.esc_html($item->post_title).'</option>';
        }
        echo '</select></td></tr>';
        echo '<tr><th><label for="pdp_release_zip">Plugin ZIP</label></th><td><input type="file" id="pdp_release_zip" name="pdp_release_zip" accept=".zip,application/zip">';
        if ($attachment) {
            $url = wp_get_attachment_url($attachment);
            echo '<div class="pdp-package-current"><strong>Current package:</strong> '.esc_html($filename ?: basename((string)get_attached_file($attachment))).' ';
            if ($url) { echo '<a href="'.esc_url($url).'" target="_blank" rel="noopener">View file</a>'; }
            echo '<br><small>'.esc_html($plugin_name ?: 'WordPress plugin').' · '.esc_html($version ?: 'Version not detected').' · '.$downloads.' licensed downloads</small></div>';
        } else {
            echo '<p class="description">Choose a WordPress plugin ZIP. Maximum size: 100 MB.</p>';
        }
        echo '</td></tr>';
        echo '<tr><th><label for="pdp_release_version">Version</label></th><td><input class="regular-text" id="pdp_release_version" name="pdp_release_version" value="'.esc_attr($version).'" placeholder="Automatically detected from ZIP"><p class="description">Leave blank to use the version found in the plugin header.</p></td></tr>';
        ...
        if ($sha256) {
            echo '<details><summary>Package integrity</summary><p><code>SHA-256: '.esc_html($sha256).'</code></p></details>';
        }
        echo '</div>';
    }
    ...
    public static function save_release($post_id) {
        ...
        $product_id = absint($_POST['pdp_release_product'] ?? 0);
        $channel = in_array(($_POST['pdp_release_channel'] ?? ''), array('stable','beta'), true) ? $_POST['pdp_release_channel'] : 'stable';
        $version = self::clean_version($_POST['pdp_release_version'] ?? '');
        $package = array();

        if (!empty($_FILES['pdp_release_zip']['name'])) {
            $package = self::validate_uploaded_package($_FILES['pdp_release_zip']);
            if (is_wp_error($package)) {
                self::set_notice($package->get_error_message(), 'error');
                update_post_meta($post_id, '_pdp_release_published', '0');
                return;
            }
            if (!$version) { $version = $package['version']; }
            $attachment_id = self::store_uploaded_package($_FILES['pdp_release_zip'], $post_id);
            if (is_wp_error($attachment_id)) {
                self::set_notice($attachment_id->get_error_message(), 'error');
                update_post_meta($post_id, '_pdp_release_published', '0');
                return;
            }
            $old_attachment = absint(get_post_meta($post_id, '_pdp_release_attachment', true));
            update_post_meta($post_id, '_pdp_release_attachment', $attachment_id);
            update_post_meta($post_id, '_pdp_release_plugin_name', $package['plugin_name']);
            update_post_meta($post_id, '_pdp_release_filename', sanitize_file_name($_FILES['pdp_release_zip']['name']));
            update_post_meta($post_id, '_pdp_release_sha256', hash_file('sha256', get_attached_file($attachment_id)));
            update_post_meta($post_id, '_pdp_release_package_size', absint($_FILES['pdp_release_zip']['size']));
            if ($old_attachment && $old_attachment !== $attachment_id) { wp_delete_attachment($old_attachment, true); }
        }

        if (!$product_id) {
            self::set_notice('Select a product before publishing the release.', 'error');
            update_post_meta($post_id, '_pdp_release_published', '0');
        }
        if (!$version) {
            self::set_notice('The release version could not be detected. Enter a version manually or upload a ZIP with a valid plugin header.', 'error');
            update_post_meta($post_id, '_pdp_release_published', '0');
        }
        if (!absint(get_post_meta($post_id, '_pdp_release_attachment', true))) {
            self::set_notice('Upload a valid plugin ZIP before making this release available.', 'error');
            update_post_meta($post_id, '_pdp_release_published', '0');
        }

        if ($product_id && $version && self::duplicate_release_exists($post_id, $product_id, $version, $channel)) {
            self::set_notice('A release with this product, version, and channel already exists. Edit the existing release or choose another version.', 'error');
            update_post_meta($post_id, '_pdp_release_published', '0');
        }

        update_post_meta($post_id, '_pdp_release_product', $product_id);
        update_post_meta($post_id, '_pdp_release_version', $version);
        update_post_meta($post_id, '_pdp_release_channel', $channel);
        ...
        $can_publish = $product_id && $version && absint(get_post_meta($post_id, '_pdp_release_attachment', true)) && !self::duplicate_release_exists($post_id, $product_id, $version, $channel);
        update_post_meta($post_id, '_pdp_release_published', isset($_POST['pdp_release_published']) && $can_publish ? '1' : '0');

        if ($can_publish && !empty($package)) {
            self::set_notice('Release package uploaded and validated successfully. Detected '.$package['plugin_name'].' version '.$version.'.', 'success');
        }
    }

    private static function validate_uploaded_package($file) {
        if (!empty($file['error'])) {
            return new WP_Error('pdp_upload_error', 'The ZIP upload failed. Upload error code: '.absint($file['error']).'.');
        }
        if (empty($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
            return new WP_Error('pdp_upload_missing', 'The uploaded package could not be verified. Please select the ZIP again.');
        }
        if (absint($file['size']) < 1 || absint($file['size']) > self::MAX_PACKAGE_BYTES) {
            return new WP_Error('pdp_upload_size', 'The plugin ZIP must be smaller than 100 MB.');
        }
        if (strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)) !== 'zip') {
            return new WP_Error('pdp_upload_type', 'Only .zip plugin packages are accepted.');
        }
        if (!class_exists('ZipArchive')) {
            return new WP_Error('pdp_zip_unavailable', 'Your server needs the PHP Zip extension to inspect plugin packages.');
        }

        $zip = new ZipArchive();
        if ($zip->open($file['tmp_name']) !== true) {
            return new WP_Error('pdp_invalid_zip', 'The selected file is not a readable ZIP archive.');
        }
        if ($zip->numFiles > 10000) {
            $zip->close();
            return new WP_Error('pdp_zip_files', 'The ZIP contains too many files to be processed safely.');
        }

        $found = array();
        for ($i = 0; $i < $zip->numFiles; $i++) {
            $name = $zip->getNameIndex($i);
            if (!$name || substr($name, -4) !== '.php' || substr_count($name, '/') > 2) continue;
            $contents = $zip->getFromIndex($i, 16384);
            if (!$contents) continue;
            if (preg_match('/^[ \t\/*#@]*Plugin Name:\s*(.+)$/mi', $contents, $name_match)) {
                preg_match('/^[ \t\/*#@]*Version:\s*(.+)$/mi', $contents, $version_match);
                $found = array(
                    'plugin_name' => sanitize_text_field(trim($name_match[1])),
                    'version' => self::clean_version(isset($version_match[1]) ? trim($version_match[1]) : ''),
                    'main_file' => sanitize_text_field($name),
                );
                break;
            }
        }
        $zip->close();
        if (empty($found['plugin_name'])) {
            return new WP_Error('pdp_not_plugin', 'No WordPress plugin header was found in the ZIP. Upload the installable plugin ZIP, not a source-code archive.');
        }
        if (empty($found['version'])) {
            return new WP_Error('pdp_no_version', 'The plugin header does not contain a valid Version value.');
        }
        return $found;
    }

    private static function store_uploaded_package($file, $post_id) {
        require_once ABSPATH . 'wp-admin/includes/file.php';
        require_once ABSPATH . 'wp-admin/includes/media.php';
        require_once ABSPATH . 'wp-admin/includes/image.php';
        $overrides = array('test_form' => false, 'mimes' => array('zip' => 'application/zip'));
        $uploaded = wp_handle_upload($file, $overrides);
        if (!empty($uploaded['error'])) return new WP_Error('pdp_store_error', $uploaded['error']);
        $attachment = array(
        ...
        update_attached_file($attachment_id, $uploaded['file']);
        return absint($attachment_id);
    }

    private static function clean_version($version) {
        return preg_replace('/[^0-9A-Za-z.\-+]/', '', sanitize_text_field(wp_unslash($version)));
    }

    private static function duplicate_release_exists($post_id, $product_id, $version, $channel) {
        $ids = get_posts(array(
            ...
            'meta_query' => array(
                array('key'=>'_pdp_release_product','value'=>absint($product_id),'compare'=>'='),
                array('key'=>'_pdp_release_version','value'=>$version,'compare'=>'='),
                array('key'=>'_pdp_release_channel','value'=>$channel,'compare'=>'='),
            ),
        ));
        return !empty($ids);
    }

    private static function set_notice($message, $type='info') {
        set_transient('pdp_release_notice_' . get_current_user_id(), array('message'=>$message,'type'=>$type), 60);
    }

    public static function admin_notices() {
        if (!current_user_can('manage_options')) return;
        $notice = get_transient('pdp_release_notice_' . get_current_user_id());
        if (!$notice) return;
        delete_transient('pdp_release_notice_' . get_current_user_id());
        $class = $notice['type'] === 'error' ? 'notice-error' : ($notice['type'] === 'success' ? 'notice-success' : 'notice-info');
    ...
    public static function latest_release($slug, $channel='stable') {
        ...
        return array(
            ...
            'sha256'=>get_post_meta($best, '_pdp_release_sha256', true),
        );
    }
    ...
    public static function dashboard() {
        ...
        $downloads = 0;
        foreach ($releases as $release) { $downloads += absint(get_post_meta($release->ID, '_pdp_release_downloads', true)); }
        global $wpdb;
        ...
        echo '<div class="wrap pdp-admin-wrap"><div class="pdp-admin-hero"><div><span class="pdp-admin-kicker">COMMERCIAL DELIVERY CENTER</span><h1>Products & Releases</h1><p>Upload installable plugin ZIPs directly, publish licensed releases, and monitor delivery activity.</p></div><div class="pdp-admin-hero-actions"><a class="button" href="'.esc_url(admin_url('edit.php?post_type='.self::PRODUCT_TYPE)).'">Manage Products</a><a class="button button-primary" href="'.esc_url(admin_url('post-new.php?post_type='.self::RELEASE_TYPE)).'">+ New Release</a></div></div>';
        echo '<div class="pdp-admin-stats"><article><span>Products</span><strong>'.count($products).'</strong><small>Commercial products configured</small></article><article><span>Releases</span><strong>'.count($releases).'</strong><small>Stable and beta packages</small></article><article><span>Licensed downloads</span><strong>'.$downloads.'</strong><small>Secure package deliveries</small></article><article><span>API activity today</span><strong>'.$requests_today.'</strong><small>'.$active_sites.' active websites</small></article></div>';
        echo '<div class="pdp-admin-card"><div class="pdp-admin-card-head"><div><h2>Release channels</h2><p>Latest installable packages currently available to licensed WordPress sites.</p></div><a class="button" href="'.esc_url(admin_url('edit.php?post_type='.self::RELEASE_TYPE)).'">Version history</a></div>';
    ...
    public static function handle_download() {
        ...
        if (get_post_meta($release_id, '_pdp_release_published', true) !== '1') wp_die('This release is not currently available.', 'Download unavailable', array('response'=>403));
        ...
        update_post_meta($release_id, '_pdp_release_downloads', absint(get_post_meta($release_id, '_pdp_release_downloads', true)) + 1);
        nocache_headers();
    ...
    public static function release_columns($columns) { return array('cb'=>$columns['cb'],'title'=>'Release','pdp_product'=>'Product','pdp_version'=>'Version','pdp_channel'=>'Channel','pdp_package'=>'Package','pdp_downloads'=>'Downloads','date'=>'Date'); }
    public static function release_column($column,$post_id) {
        if($column==='pdp_product') echo esc_html(get_the_title(absint(get_post_meta($post_id,'_pdp_release_product',true))));
        if($column==='pdp_version') echo '<code>'.esc_html(get_post_meta($post_id,'_pdp_release_version',true)).'</code>';
        if($column==='pdp_channel') echo esc_html(ucfirst(get_post_meta($post_id,'_pdp_release_channel',true)?:'stable'));
        if($column==='pdp_package') { $filename=get_post_meta($post_id,'_pdp_release_filename',true); echo absint(get_post_meta($post_id,'_pdp_release_attachment',true)) ? '<strong>Ready</strong><br><small>'.esc_html($filename).'</small>' : '<span style="color:#b32d2e">Missing</span>'; }
        if($column==='pdp_downloads') echo absint(get_post_meta($post_id,'_pdp_release_downloads',true));
    }
}
